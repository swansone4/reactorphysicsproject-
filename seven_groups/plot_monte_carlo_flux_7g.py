...
def main() -> int:
    ...
    if not csv_flux.is_file():
        raise FileNotFoundError(f"Missing {csv_flux}. Run monte_carlo_solver_7g first.")
    if not csv_cells.is_file():
        raise FileNotFoundError(f"Missing {csv_cells}. Run monte_carlo_solver_7g first.")
    ...
    ax.set_title("Seven-group Monte Carlo flux (Groups 1-7)")
    ax.grid(alpha=0.25)
    ax.legend(loc="upper center", ncol=4, fontsize=9, frameon=True)
    fig.tight_layout()
    out_png = out_dir / "mc_flux_g1_to_g7.png"
    ...
    ax2.set_ylabel("Cell-center scalar flux")
    ax2.set_title("Seven-group cell-center flux")
    ax2.grid(alpha=0.25)
    ax2.legend(loc="upper center", ncol=4, fontsize=9, frameon=True)
    fig2.tight_layout()
    out_centers = out_dir / "mc_cell_center_flux_g1_to_g7.png"
    fig2.savefig(out_centers, dpi=200)
    plt.close(fig2)

    # Cell-edge current from finite-volume/Fick interface closure:
    # d_i,g = D_i,g / dx_i,  D_i,g = 1 / (3*SigTR_i,g)
    # J_{i+1/2,g} = - [2 d_i,g d_{i+1,g} / (d_i,g + d_{i+1,g})] * (phi_{i+1,g} - phi_{i,g})
    # This follows the lecture derivation using a shared edge flux phi_{i+1/2}.
    sigTR = np.array(
    ...
    ax3.set_ylabel(r"Cell-edge current $J(x)$")
    ax3.set_title(
        r"Seven-group cell-edge current: "
    ...
    fig3.tight_layout()
    out_edges = out_dir / "mc_cell_edge_current_g1_to_g7.png"
    fig3.savefig(out_edges, dpi=200)
    plt.close(fig3)

    for legacy_name in (
        "mc_flux_g1_g7.png",
        "mc_current_profile_ficks_law.png",
        "mc_current_profile_edges.png",
        "mc_cell_edge_flux_g1_to_g7.png",
        "mc_cell_edge_current_g1_vs_g7.png",
    ):
        legacy_png = out_dir / legacy_name
